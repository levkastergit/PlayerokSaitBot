...
const SETTINGS_URL = `${BACKEND_ORIGIN}/api/clode/settings`

const FETCH_CREDENTIALS = { credentials: 'include' }

export async function fetchClodeSettings() {
...
export async function saveClodeApiKey(apiKey) {
...
export async function clearClodeApiKey() {
  const response = await fetch(SETTINGS_URL, {
    ...FETCH_CREDENTIALS,
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
